...
# ★ ‘차선 내부’ 코리더 폭(자차 기준 좌우 half-width) — in-lane 전용 게이팅
LANE_LATERAL_LIMIT_M       = 2.0   # = 차선폭 약 4.0 m 가정. 필요시 1.7~2.1로 조정
...
class TargetTracker:
    ...
    def update_from_radar(self, radar_measurement: carla.RadarMeasurement, sim_time: float,
        ...
        for det in radar_measurement:
            ...
            # ★ in-lane 게이팅: 자차 기준 횡방향(|y|)이 차선 코리더 안일 때만 후보
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # ===== Radar tracking (original logic + in-lane 게이팅) =====
            update_ok = False
            if _radar_q:
            ...
            acc_payload = {
                ...
                'sim_time': sim_time,
                # ★ has_target는 이미 in-lane 게이팅을 통과한 표적에 대해서만 TRUE
