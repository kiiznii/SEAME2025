...
try:
    ...
    # Fallback to local LK_algo.py if provided
    from LK_algo import (
        detect_lanes_and_center, fuse_lanes_with_memory,
        lane_mid_x, lookahead_ratio, speed_of
    )

# ========= ORIGINAL radar parameters =========
MIN_VALID_DEPTH            = 0.5
...
# ========= (ADDED) Transform helpers (world <-> sensor) =========
def transform_to_matrix(T: carla.Transform) -> np.ndarray:
    """carla.Transform -> 4x4 world transform matrix [R|t]"""
    r = T.rotation
    l = T.location
...
def world_to_sensor(p_world: carla.Location, sensor_T: carla.Transform) -> np.ndarray:
    """World point -> Sensor-local (X,Y,Z)"""
    Mw = transform_to_matrix(sensor_T)   # World <- Sensor
    Ms = np.linalg.inv(Mw)               # Sensor <- World
    pw = np.array([p_world.x, p_world.y, p_world.z, 1.0], dtype=np.float64)
    ps = Ms @ pw
    return ps[:3]

# ========= (ADDED) Chase overlay: true projection =========
def draw_radar_on_chase_rays(
    ...
    thickness: int = 2,
    # ---- 게이팅 파라미터 ----
    min_depth_m: float = 6.0,        # 자차 근거리 클러터 컷 (필요시 4~8m 범위에서 조절)
    max_depth_m: float = 150.0,
    max_lat_m: float = 2.2,          # 차로 중앙 코리더(|y|<=2.2m)
    max_alt_deg: float = 6.0,        # 고도각 제한(|alt|<=6°)
    require_approach: bool = True,   # 접근(velocity<0)만 남기기 → 앞차/전방 이동물체에 포커스
    start_t: float = 0.12            # 선 시작점 보정(원점→타깃 선분의 t지점부터 그림)
) -> np.ndarray:
    """
    레이더 검출을 카메라로 투영해 '센서 원점→타깃' 선분으로 그린다.
    - 근거리/지면/차로 밖/정지물 클러터를 게이팅해서 '전방 물체'에 집중.
    - start_t로 보닛/루프 겹침을 줄임.
    """
    ...
    # 레이더 원점 투영(보이면 그 점을 원점으로, 아니면 화면 하단 중앙 근처 대체)
    ...
    for d in radar_meas:
        ...
        if not (min_depth_m <= depth <= max_depth_m):
            continue
        if require_approach and not (d.velocity < 0):  # 접근만
            continue
        az  = float(d.azimuth)   # rad
        alt = float(d.altitude)  # rad
        if abs(alt) > max_alt:
            continue

        # 레이더 로컬 좌표 (X fwd, Y right, Z up)
        xr = depth * math.cos(alt) * math.cos(az)
        yr = depth * math.cos(alt) * math.sin(az)
        zr = depth * math.sin(alt)

        # 차로 코리더 게이팅(|y|<=max_lat_m)
        if abs(yr) > max_lat_m:
            continue
...
def main():
    ...
    try:
        ...
        chase_bp.set_attribute('fov', '70')  # can tune
