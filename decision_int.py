...
# - FOLLOW 진입: has_target 연속≥2 AND distance<80m
# - AEB는 타깃 확정 시에만
...
def main():
    ...
    try:
        while True:
            ...
            # FOLLOW 표적 게이팅
            target_ready = (target_stable_frames >= MIN_CONFIRM_FRAMES) and (distance < FOLLOW_MAX_DIST_M)
            ...
            if current_mode != "AEB":
                ...
                # ★ 크루즈 부스트: 타깃 없거나 TTC 비유효(표시 -1)면 80km/h까지 더 적극 가속
                if (not target_ready):
                    speed_gap = max(0.0, TARGET_SPEED_MPS - v_mps)  # m/s
                    # 기본 스로틀 바닥을 보장(속도차에 비례해 0.12~0.6 사이)
                    boost = np.clip(0.12 + 0.06 * speed_gap, 0.12, 0.60)
                    throttle_des = max(throttle_des, float(boost))
                    brake_des = 0.0  # 불필요 제동 방지
